...
from app.bootstrap.tempo_data import APPROVED_TABLES, INVENTORY_SPEC, SALES_SPEC, TempoFixtureBundle
...
def _qualified(catalog: str, schema: str, table: str) -> str:
    if table not in APPROVED_TABLES or not all(_IDENTIFIER.fullmatch(part) for part in (catalog, schema, table)):
...
def validate_loader_sql(sql: str, catalog: str, schema: str) -> None:
    ...
    if qualified.lower() not in {_qualified(catalog, schema, table).lower() for table in APPROVED_TABLES}:
        raise LoaderSafetyError("LOADER_TARGET_REJECTED")
